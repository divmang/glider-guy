...
export default function GliderGuy() {
  ...
  function draw(ctx, g) {
    ...
    skyG.addColorStop(0,"#07122a"); skyG.addColorStop(0.6,"#0d2040"); skyG.addColorStop(1,"#122818");
    ctx.fillStyle=skyG; ctx.fillRect(0,0,W,groundY);

    // moon + stars
    ctx.fillStyle="#fff6d0"; ctx.beginPath(); ctx.arc(W*0.82,groundY*0.15,22,0,Math.PI*2); ctx.fill();
    for(let i=0;i<48;i++){const sx=((i*137+17)%97)/97*W,sy=((i*89+43)%91)/91*groundY*0.80;ctx.globalAlpha=(0.3+0.7*Math.abs(Math.sin(f*0.022+i)))*0.55;ctx.fillStyle="#fff";ctx.beginPath();ctx.arc(sx,sy,1.1,0,Math.PI*2);ctx.fill();}
    ctx.globalAlpha=1;

    // clouds
    const cOff=(f*0.16)%W; ctx.fillStyle="#ffffff0b";
    for(const[rx,ry,cw,ch]of[[.08,.06,65,16],[.38,.04,90,20],[.65,.10,58,15],[.22,.17,72,14]]){const cx=((rx*W+cOff)%W);ctx.beginPath();ctx.ellipse(cx,ry*groundY,cw,ch,0,0,Math.PI*2);ctx.fill();ctx.beginPath();ctx.ellipse(cx-W,ry*groundY,cw,ch,0,0,Math.PI*2);ctx.fill();}

    // hills
    const hB=groundY,hMH=groundY*0.50;
    ctx.fillStyle="#163820"; ctx.beginPath(); ctx.moveTo(0,hB);
    for(const[rx,rh]of[[0,.5],[.08,.7],[.16,.46],[.26,.86],[.34,.58],[.44,.92],[.53,.60],[.63,.76],[.72,.48],[.82,.68],[.92,.42],[1,.55]])ctx.lineTo(rx*W,hB-rh*hMH);
    ctx.lineTo(W,hB); ctx.closePath(); ctx.fill();
    ctx.fillStyle="#1b4828"; ctx.beginPath(); ctx.moveTo(-2,hB); ctx.lineTo(-2,hB-hMH*0.46); ctx.lineTo(W*0.06,hB-hMH*0.70); ctx.lineTo(W*0.14,hB-hMH*0.50); ctx.lineTo(W*0.22,hB); ctx.closePath(); ctx.fill();
    ctx.fillStyle="#ddeeff"; ctx.beginPath(); ctx.moveTo(W*0.06,hB-hMH*0.70); ctx.lineTo(W*0.036,hB-hMH*0.60); ctx.lineTo(W*0.084,hB-hMH*0.60); ctx.closePath(); ctx.fill();

    // button zone background (dark strip)
    const btnG=ctx.createLinearGradient(0,groundY,0,H);
    btnG.addColorStop(0,"#0a100a"); btnG.addColorStop(1,"#050805");
    ctx.fillStyle=btnG; ctx.fillRect(0,groundY,W,BTN_ZONE_H);
    ctx.fillStyle="#4ec02222"; ctx.fillRect(0,groundY,W,2);

    // pillars
    for (const p of pillars) {
      const botY=p.topH+p.gap, botH=groundY-botY;
      const pb=(px,py,ph)=>{if(ph<=0)return;const pg=ctx.createLinearGradient(px,0,px+PILLAR_W,0);pg.addColorStop(0,"#185c38");pg.addColorStop(.28,"#2ea055");pg.addColorStop(.72,"#228844");pg.addColorStop(1,"#124a2c");ctx.fillStyle=pg;rr(ctx,px,py,PILLAR_W,ph,4);ctx.fill();ctx.fillStyle="#55ff8818";ctx.fillRect(px+3,py,5,ph);};
      const pc=(px,py)=>{ctx.fillStyle="#0d381a";rr(ctx,px-8,py,PILLAR_W+16,20,5);ctx.fill();ctx.fillStyle="#38a860";ctx.fillRect(px-8,py,PILLAR_W+16,5);};
      pb(p.x,0,p.topH); pc(p.x,p.topH-20);
      if(botH>0){pb(p.x,botY,botH);pc(p.x,botY);}
    }

    // blasts
    for(const b of blasts){ctx.globalAlpha=Math.max(0,b.life);ctx.fillStyle=`hsl(${b.hue},100%,72%)`;ctx.shadowColor=`hsl(${b.hue},100%,60%)`;ctx.shadowBlur=12;ctx.beginPath();ctx.arc(b.x,b.y,b.r,0,Math.PI*2);ctx.fill();}
    ctx.globalAlpha=1; ctx.shadowBlur=0;

    // particles
    for(const p of particles){ctx.globalAlpha=Math.max(0,p.life);ctx.fillStyle=`hsl(${p.hue},100%,68%)`;ctx.beginPath();ctx.arc(p.x,p.y,p.r*p.life,0,Math.PI*2);ctx.fill();}
    ctx.globalAlpha=1;

    // player
    if (!ply.dead) {
      const {x,y,vy}=ply;
      ctx.save(); ctx.translate(x,y); ctx.rotate(Math.max(-0.40,Math.min(0.40,vy*0.042)));
      const wa=Math.sin(f*0.13)*0.09;
      for(const side of[-1,1]){ctx.save();ctx.rotate(side*wa);ctx.fillStyle="#ff5500cc";ctx.beginPath();ctx.moveTo(0,2);ctx.lineTo(side*-44,-7);ctx.lineTo(side*-36,13);ctx.closePath();ctx.fill();ctx.strokeStyle="#cc2200";ctx.lineWidth=1.5;ctx.beginPath();ctx.moveTo(0,2);ctx.lineTo(side*-44,-7);ctx.stroke();ctx.restore();}
      ctx.fillStyle="#ee3300"; ctx.beginPath(); ctx.moveTo(-14,3); ctx.lineTo(-28,-10); ctx.lineTo(-8,8); ctx.closePath(); ctx.fill();
      ctx.strokeStyle="#ffdd00"; ctx.lineWidth=3; ctx.lineCap="round";
      ...
      ctx.fillStyle="#ee3322"; ctx.beginPath(); ctx.arc(9,-4,8.5,Math.PI,0); ctx.fill();
      ctx.fillStyle="#aa1100"; ctx.beginPath(); ctx.ellipse(9,-4,8.5,3,0,Math.PI,0); ctx.fill();
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              cursor:"pointer",
              boxShadow:"0 4px 0 #004499, 0 0 20px #00aaff66",
          ...
          {/* DISABLED button — placeholder for future feature */}
          <button
            disabled
            style={{
              flex:1, height:70,
              ...
              borderRadius:16,
              color:"#ffffff33", fontSize:18, fontWeight:700,
              cursor:"not-allowed",
              display:"flex", alignItems:"center", justifyContent:"center", gap:8,
              touchAction:"none",
            }}>
